...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# your_code
# CentralAir : NA most likely means no air conditioning
replace_na(df, "CentralAir", value="No")
# EnclosedPorch : NA most likely means zero square feet of enclosed poarch
replace_na(df, "EnclosedPorch", value=0)
# Fireplaces :  NA most likely means zero fireplaces
...
def assess_regression_model(model, X_train, X_test, y_train, y_test) -> tuple[float, float]:
    ...
    rmse_train = root_mean_squared_error(y_train_exp, y_pred_train_exp)
    rmse_test = root_mean_squared_error(y_test_exp, y_pred_test_exp)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# błąd w zbiorze testowym jest około 35% większy względem błędu na zbiorze treningowym, więc występuje przeuczenie. Błędy to odpowiednio 16727.27 na zbiorze trenignowym i 21448.86 na zbiorze testowym, więc względem cen wyrażonych w setkach tysięcy nie jest przesadnie duży
#
#
...
#L1 Regularization
lasso_cv = LassoCV(n_alphas=1000,random_state=0)
lasso_cv.fit(X_train, y_train)

reg_ridge_alpha = round(ridge_cv.alpha_, 4)
reg_lasso_alpha = round(lasso_cv.alpha_, 4)
print("ridge_alpha: ", reg_ridge_alpha)
print("lasso_alpha: ", reg_lasso_alpha)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Po skorzystaniu z regularyzacji problem overfittingu zniknął, minimalnie lepsze wyniki daje regularyzacja L2
#
#
...
#filtering out the illetares
df = df.loc[df['education'] != 'illiterate' , :]

#replacing the data with mappings provided
df = df.replace(
    {
...
#changing data types after replacment
df = df.astype({
    "contact": np.int64,
    "month": np.int64,
...
"""# Age : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Job : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Marital : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Housing : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Education : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Loan : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Contact : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Month : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# day_of_week : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Campaign : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Previous : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Emp.var.rate : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Cons.price.idx : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Cons.conf.idx : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Euribor3m : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")
# Nr.employed : data description says NA means "no alley access"
replace_na(df, "Alley", value="None")"""

# Nie ma sensownych metod uzupełnienia danych, bo brakuje ich we wszystkich rekordach, więc nie da się w żaden sposób uśrednić, a nie ma żadnych narzucających
# się danych które można podstawić za NA
...
ax = y.value_counts().plot(kind="bar", color=["#890014", "#0a5c0a"])
...
numerical_features = ['age', 'contact', 'month', 'day_of_week', 'campaign', 'previous',
...
column_transformer = ColumnTransformer(
    [
        ...
        ('numerical', standard_scaler, numerical_features)
...
def asses_regression_logistic_model(model, X_train, X_test, y_train, y_test)-> tuple[float, float, float]:
    ...
    # print the results
    print("precision_test: ", round(precision *100,2), "%")
    print("recall_test: ", round(recall *100,2), "%")
...
log_reg_l0 = LogisticRegression(random_state = 0, class_weight="balanced", penalty=None)

print("test results: ")
l0_precision, l0_recall, l0_f1 = asses_regression_logistic_model(log_reg_l0, X_train, X_test, y_train, y_test)

# %%
from sklearn.linear_model import LogisticRegressionCV


log_reg_l2 = LogisticRegressionCV(penalty='l2', Cs=100, cv=5, class_weight='balanced',random_state = 0, n_jobs = -1, scoring='f1')

l2_precision, l2_recall, l2_f1 = asses_regression_logistic_model(log_reg_l2, X_train, X_test, y_train, y_test)

# %%
from sklearn.linear_model import LogisticRegressionCV


log_reg_l1 = LogisticRegressionCV(penalty = 'l1',  Cs=100, cv=5, class_weight='balanced', random_state = 0, solver = "saga", n_jobs = -1, scoring='f1')

l1_precision, l1_recall, l1_f1 = asses_regression_logistic_model(log_reg_l1, X_train, X_test, y_train, y_test)
...
X_train = numerical_pipeline.fit_transform(X_train)
X_test = numerical_pipeline.transform(X_test)

log_reg_l0 = LogisticRegression(random_state = 0, class_weight="balanced", penalty=None, max_iter = 200)
log_reg_l0.fit(X_train, y_train)

y_pred_train = log_reg_l0.predict(y_train)
y_pred_test = log_reg_l0.predict(y_test)
